...
// * 1.0 Для начала создадим функцию getData(), которая будет получать данные из локальной базы данных users.json и возвращать его в цепочку then().
...
// * 2.2 Но у "async" есть также пара "await", которое используется вместе с ним, рассмотрим их работу на след. примере. Для этого внутри тела функции "getData()" объявим переменную "response", в которой будет лежать результат выполнения метода fetch(), а обращаться он будет к нашей мини базе данных "users.json".
